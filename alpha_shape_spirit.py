    ...
    try:
        ...
        if 'alpha_result' in st.session_state:
            ...
            with st.expander("🎨 Visualización 3D - Nanoporos Detectados", expanded=True):
                ...
                with col_viz2:
                    show_surface = st.checkbox("Mostrar átomos Nanoporo", value=True)
                with col_viz3:
                    marker_size = st.slider("Tamaño de átomos", 1, 10, 4)
                ...
                if show_bulk:
                    ...
                    df_to_plot.append(df_all_atoms)
                
                if show_surface:
                    df_filtered_copy = filtered_df.copy()
                    df_filtered_copy['Tipo'] = 'Nanoporo'
                    df_to_plot.append(df_filtered_copy)
