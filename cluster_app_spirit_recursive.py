...
# -------------------------------------------------------------------------
# (3) *** CLUSTERING ENGINE CON RECURSIVIDAD ***
# -------------------------------------------------------------------------
...
class ClusteringEngine:
    ...
    def aplicar_meanshift(self, quantile=0.2):
        bw = estimate_bandwidth(self.coords, quantile=quantile, n_samples=min(50,len(self.coords)))
    ...
    def compute_metrics(self):
        ...
        self.metrics = {}  # Inicializar métricas
        
        # Para HDBSCAN, excluir ruido (-1)
        valid = self.labels != -1
        X = self.coords[valid]
        ...
        if len(X) > 0 and len(np.unique(labels)) > 1:
            try:
                ...
                st.warning(f"⚠️ No se pudieron calcular algunas métricas: {str(e)}")
        
        # Contar puntos de ruido si es HDBSCAN
        if self.method == "HDBSCAN":
            noise_count = (self.labels == -1).sum()
            self.metrics['noise_points'] = noise_count
            self.metrics['noise_percentage'] = 100 * noise_count / len(self.labels)
    ...
    def aplicar_hdbscan(self, min_cluster_size=10, min_samples=None):
        ...
        self.data_df['Cluster'] = self.labels
        return len(np.unique(self.labels[self.labels != -1]))  # Excluir ruido

    # ===================== RECURSIVIDAD =====================
    def recursive_clustering(self, max_size=5000, depth=0, prefix=""):
        df = self.data_df.copy()

        # Si el cluster es chico → no subdividir
        if len(df) <= max_size:
            df['Cluster_Level'] = depth
            df['Cluster'] = prefix if prefix else "0"
            return df

        # Ejecutar clustering del nivel actual
        if self.method == "KMeans":
            self.aplicar_kmeans(**self.method_params)
        elif self.method == "MeanShift":
            self.aplicar_meanshift(**self.method_params)
        elif self.method == "Aglomerativo":
            self.aplicar_aglomerativo(**self.method_params)
        elif self.method == "HDBSCAN":
            self.aplicar_hdbscan(**self.method_params)
        ...
        for c in sorted(df['Cluster'].unique()):
            ...
            new_prefix = f"{prefix}_{c}" if prefix else str(c)

            if len(sub) > max_size:
                sub_engine = ClusteringEngine((self.header, sub))
                sub_engine.method = self.method
                sub_engine.method_params = self.method_params
                res = sub_engine.recursive_clustering(max_size=max_size, depth=depth+1, prefix=new_prefix)
                output.append(res)
            else:
                sub['Cluster_Level'] = depth
                sub['Cluster'] = new_prefix
                output.append(sub)
...
if uploaded_file is None:
    ...
    try:
        ...
        with st.sidebar:
            ...
            recursive_enabled = st.sidebar.checkbox("Clustering recursivo", value=False)
            max_cluster_size = st.sidebar.number_input(
                "Tamaño máximo de cluster antes de subdividir",
                min_value=50,
                max_value=200000,
                value=5000,
                step=50
        ...
        if run_button:
            engine = ClusteringEngine((header, df.copy()))
            engine.method = clustering_method
            engine.method_params = method_params
            engine.leer_dump()

            if recursive_enabled:
                clustered_df = engine.recursive_clustering(
                    max_size=max_cluster_size,
                    depth=0
                )
                n_clusters_found = len(clustered_df['Cluster'].unique())
            else:
                if clustering_method == "KMeans":
                ...
                clustered_df = engine.data_df

            # Calcular métricas si está habilitado
            if enable_metrics:
            ...
            st.session_state['clustering_result'] = {
                ...
                'metrics': engine.metrics  # Guardar métricas calculadas
            }
        ...
        if 'clustering_result' in st.session_state:
            ...
            # Calcular tamaños de clusters (excluyendo ruido)
            ...
            if len(cluster_sizes) > 0:
                ...
                col3.metric("Cluster más grande", "N/A")
                col4.metric("Cluster más pequeño", "N/A")
            ...
            if enable_metrics and result['metrics']:
                ...
                if 'silhouette' in result['metrics']:
                    ...
                    metrics_cols[0].metric("Silhouette Score", "N/A")
                ...
                if 'davies_bouldin' in result['metrics']:
                    ...
                    metrics_cols[1].metric("Davies-Bouldin Index", "N/A")
                ...
                if 'calinski_harabasz' in result['metrics']:
                    ...
                    metrics_cols[2].metric("Calinski-Harabasz Index", "N/A")
            ...
            with st.expander("📋 Estadísticas detalladas por cluster", expanded=True):
                # Asegurarse de que tenemos cluster_sizes definido
                ...
                if len(cluster_sizes) > 0:
                    ...
                    st.info("No hay clusters válidos para mostrar estadísticas")
