#!/usr/bin/env python3
"""
CLI para extracción de features
"""
...
from unified_md_analysis.core.feature_extraction import extract_all_features
from unified_md_analysis.utils.lammps_parser import LAMMPSDumpParser


def main():
    parser = argparse.ArgumentParser(
        description="Extracción de features para Machine Learning",
...
Ejemplos:
  ...
  python extract.py dumps/ --output features.csv

  # Con target conocido (n_vacancies)
  python extract.py dumps/ --output features.csv --vacancies-file vacancies.txt
        """
    )
    ...
    print(f"\n{'='*80}")
    print(f"{'EXTRACCIÓN DE FEATURES':^80}")
    print(f"{'='*80}\n")
